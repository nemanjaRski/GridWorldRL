...
def get_policy_model(env, hidden_layer_neurons, lr):
    ...
    model_train.compile(loss="mse", optimizer=Adam(lr))
...
while num_episode < num_episodes:
    ...
    observation, reward, done, _ = env.step(action)
    if observation[0] > episode_max_height:
        episode_max_height = observation[0]

    rewards = np.vstack([rewards, 0])

    if done:
        reward_sum += episode_max_height
        rewards[-1] = episode_max_height
        # print(episode_max_height)
        episode_max_height = -10
